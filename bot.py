    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL4:
            try:
                ...
                self.LOGGER(__name__).warning(
                    f"Silakan periksa kembali var FORCE_SUB_CHANNEL4 dan Pastikan Bot anda Admin di Channel dengan izin link invite Pengguna melalui link undangan, Subs Channel Saat Ini: {FORCE_SUB_CHANNEL4}"
                )
                self.LOGGER(__name__).info(
                    "\nBot Berhenti. Tanya ke https://t.me/SharingUserbot untuk Bantuan"
                )
